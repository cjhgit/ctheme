 ...
 * Description: 目录图片，支持默认的文章类型，也支持自定义文章类型，同时还支持多站点模式
 * Author: 陈建杭
 */
...
define('Z_IMAGE_PLACEHOLDER', get_template_directory_uri() . "/images/post-default.jpg");
...
function z_add_texonomy_field() {
	...
	echo '<div class="form-field">
		<label for="taxonomy_image">分类图片</label>
		<input type="text" name="taxonomy_image" id="taxonomy_image" value="" />
		<br/>
		<button class="z_upload_image_button button">上传/添加图片</button>
	</div>'.z_script();
}

// 给编辑表单添加一个图像字段（编辑分类目录那里）
function z_edit_texonomy_field($taxonomy) {
	...
	echo '<tr class="form-field">
		...
		<button class="z_upload_image_button button">上传/添加图片</button>
		<button class="z_remove_image_button button">删除图片</button>
		</td>
	</tr>'.z_script();
}
...
// get taxonomy image url for the given term_id (不加任何参数返回默认图片)
		...
		<div class="thumb inline-edit-col">
			<label>
				...
				<span class="input-text-wrap">
					<button class="z_upload_image_button button">上传/添加图片</button>
					<button class="z_remove_image_button button">移除图片</button>
				</span>
			</label>
		</div>
	</fieldset>';
