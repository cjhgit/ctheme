...
add_action('widgets_init', 'register_comment_widget');

function register_comment_widget() {
    register_widget('CommentWidget');
}
...
class CommentWidget extends WP_Widget {
    ...
    function __construct() {
        $widget_ops = array(
            'classname' => 'widget-comment',
            'description' => '显示网友最新评论（头像+名称+评论）'
        );
        $this->WP_Widget('git_comment', 'C-评论列表（小C主题）', $widget_ops);
    }
