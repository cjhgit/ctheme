<?php
//require get_template_directory() . '/functions/log.php'; // 如果不需要日志功能，去掉这一句
require get_template_directory() . '/functions/type.php'; // 文章类型
require get_template_directory() . '/functions/post_list.php';
require get_template_directory() . '/functions/product.php';
require get_template_directory() . '/functions/talk.php';
require get_template_directory() . '/functions/notice.php';
require get_template_directory() . '/functions/post_content.php';
require get_template_directory() . '/functions/admin.php';
require get_template_directory() . '/functions/view.php';
require get_template_directory() . '/functions/base.php';
require get_template_directory() . '/functions/comment.php';
require get_template_directory() . '/functions/categories_images.php';
//require get_template_directory() . '/functions/avatars.php';

require get_template_directory() . '/widgets/widget.php';
require get_template_directory() . '/inc/options.php';